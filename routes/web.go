...
func Web() {
	...
	// Inertia static assets:
	// - /public/* serves the committed root-template assets (e.g. public/favicon.ico).
	// - /build/* serves the Vite production bundle. public/build is NOT committed;
	//   it is generated by `npm run build` (vite.config.ts sets outDir to
	//   public/build) and gitignored. The vite template helper emits /build/... URLs
	//   from its manifest.
	...
	// Inertia demo routes. These are session-based web routes (StartSession is
	// already global). To actually see the rendered page, run the Vite dev server
	// first (`npm install && npm run dev`), which writes public/hot so the backend
	// loads JS/CSS from Vite (HMR); otherwise run `npm run build` to serve the
	// production bundle via /build.
